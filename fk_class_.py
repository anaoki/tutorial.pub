...
class Link(object):
	...
	self.th2 = th2_
    # 順運動学の計算
    def arm2_fk(self,x1,y1,x2,y2):
        # 原点座標（縦ベクトル）
        vec = np.array([[0.],[0.],[1.]] );
        # 順運動学の計算
        (x1, y1, z1) = R(self.th1)*L(self.l1)*vec;                # 第1関節の位置
        (x2, y2, z2) = R(self.th1)*L(self.l1)*R(self.th2)*L(self.l2)*vec;   # 第2関節の位置
        return x1, y1, x2, y2
...
if __name__ == '__main__':
    link = Link(0.3, 0.4, np.radians(60), np.radians(30))
    # 順運動学の計算
    (x1, y1, x2, y2) = link.arm2_fk(link.l1, link.l2, link.th1, link.th2)
    # ロボットアームの描画
    x = (0, x1, x2)
    y = (0, y1, y2)
    plot(x, y)
